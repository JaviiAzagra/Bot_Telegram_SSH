...
curl -s --max-time $TIMEOUT -d "parse_mode=HTML&disable_notification=$SONIDO&chat_id=$USERID&disable_web_page_preview=1&text=`echo -e "$TEXTO"`" $URL >> $LOG 2>&1 #Realizamos la peticion para que envie el mensaje que le indicaremos mas adelante.
